...
test.describe('Ashout logging', () => {
  ...
  test('logs an entry and shows it in the journal', async ({ page }) => {
    ...
    await expect(entry(page, 'bench press')).toBeVisible()
  ...
  test('auto-structures a workout into muscle-group and volume chips', async ({ page }) => {
    ...
    const row = entry(page, 'bench press')
  ...
  test('aggregates insights and ranks muscle groups', async ({ page }) => {
    await page.goto('/')
    await log(page, 'bench press 3x8 at 60kg')
    await expect(entry(page, 'bench press').getByText('chest')).toBeVisible()
  ...
  test('persists entries across a reload (on-device storage)', async ({ page }) => {
    ...
    await expect(entry(page, 'easy run')).toBeVisible()

    await page.reload()
    await expect(entry(page, 'easy run')).toBeVisible()
  })

  test('edits an individual set on an entry', async ({ page }) => {
    await page.goto('/')
    await log(page, 'bench press 3x8 at 60kg')

    const row = entry(page, 'bench press')
    await expect(row.getByText('chest')).toBeVisible() // wait for structuring
    await row.getByRole('button', { name: /edit entry/i }).click()

    await page.getByLabel(/set 1 reps/i).fill('10')
    await page.getByRole('button', { name: /^save$/i }).click()

    await expect(entry(page, 'bench press').getByText('10×60')).toBeVisible()
  })
